...
describe("massageTreeForFonts", () => {
  ...
  it("should read existing next/font variable names from layout for theme vars", async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "shadcn-update-fonts-"))
...
`
    ...
    vi.mocked(getProjectInfo).mockResolvedValueOnce({
      ...
      isRsc: true,
      tailwindVersion: "v4",
      tailwindConfigFile: path.join(cwd, "tailwind.config.ts"),
      tailwindCssFile: path.join(cwd, "app/globals.css"),
    ...
    const result = await massageTreeForFonts(
      {
        fonts: [
          {
            name: "font-geist",
    ...
    expect(result.cssVars?.theme?.["--font-sans"]).toBe("var(--font-geist-sans)")
