...
`
  ...
  it("should remove unused default Geist font when replaced (create-next-app scenario)", async () => {
    // This simulates a fresh create-next-app project where a preset
    // replaces the sans font with Inter. See issue #11124.
    const input = `
import type { Metadata } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import "./globals.css";
...
`
    ...
    // Geist is superseded by Inter, so its import, declaration and
    // className usage should be removed. Geist_Mono still backs
    // --font-mono, so it should be kept.
    expect(result).toMatchInlineSnapshot(`
      "
      import type { Metadata } from "next";
  ...
  it("should remove both default fonts when sans and mono are replaced", async () => {
    const input = `
import { Geist, Geist_Mono } from "next/font/google";
...
`
    ...
    expect(result).not.toContain("Geist")
    expect(result).not.toContain("geistSans")
    expect(result).not.toContain("geistMono")
    expect(result).toContain("Inter")
    expect(result).toContain("JetBrains_Mono")
  })

  it("should keep default Geist font when it is used outside className", async () => {
    const input = `
...
export const fontClass = geistSans.className;
...
`
    ...
    // geistSans is referenced beyond .variable usage, so it must be kept.
    expect(result).toContain("const geistSans = Geist({")
    expect(result).toContain("geistSans.className")
  })
