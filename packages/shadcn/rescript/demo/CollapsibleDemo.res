...
let make = () => {
  ...
  <Collapsible open_=isOpen onOpenChange={(nextOpen, _) => setIsOpen(_ => nextOpen)} className="flex w-[350px] flex-col gap-2">
    <div className="flex items-center justify-between gap-4 px-4">
      <h4 className="text-sm font-semibold">{"Order #4189"->React.string}</h4>
      <Collapsible.Trigger
        render={<Button dataVariant=BaseUi.Types.Variant.Ghost dataSize=BaseUi.Types.Size.Icon className="size-8" />}
      >
