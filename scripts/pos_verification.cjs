// pos_verification.cjs
// Usage examples:
// COOKIE='session=abc; other=def' node scripts/pos_verification.cjs
// EMAIL=test@example.com PASSWORD=TestPass123! node scripts/pos_verification.cjs
...
(async () => {
  ...
  if (!COOKIE && (!EMAIL || !PASSWORD)) {
    ...
    console.error('Example (cookie): COOKIE="session=abc; other=def" node scripts/pos_verification.cjs');
    console.error('Example (creds): EMAIL=test@example.com PASSWORD=TestPass123! node scripts/pos_verification.cjs');
    process.exit(1);
  }
  ...
  const posUrl = new URL('/erp/pos/checkout', baseUrlObj).toString();
