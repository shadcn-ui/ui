...
You can also secure your registry using other methods, such as requiring a `Bearer` token in the `Authorization` header.
...
If you're using a private registry that requires authentication (e.g. Bearer tokens or API keys), you can configure the CLI to send custom headers by adding a `.shadcnrc.json` file in the root of your project:

<Callout className="mt-6">
  **Note:** Since `.shadcnrc.json` may contain sensitive information such as authentication tokens, be sure to add it to your `.gitignore` file to prevent accidental exposure.
</Callout>

```json title=".shadcnrc.json" showLineNumbers
{
  "registries": {
    "https://my-registry.com/r1/*": {
      "headers": {
        "Authorization": "Bearer YOUR_TOKEN1"
      }
    },
    "https://my-registry.com/r2/*": {
      "headers": {
        "Authorization": "Bearer YOUR_TOKEN2"
      }
    },
...
The CLI will automatically include the defined headers when making requests to matching registry URLs. Wildcard patterns (`*`) are supported to target groups of URLs.

<Callout className="mt-6">
  **Note:** This only affects CLI usage. The UI ("Open in v0") does not currently read from `.shadcnrc.json`.
</Callout>
