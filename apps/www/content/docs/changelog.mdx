...
This file contains all the information about your components: where to install them, the import paths, how they are styled...etc. For example, you can change the import path of a component, set a baseColor or change the styling method.
...
This means you can now use the cli with any directory structure including `src` or `app` directories.
...
The CLI will automatically resolve all components and dependencies, format them to your custom config and add them to your project.

### `diff` (experimental)

We're also introducing a new `diff` command to help you keep track of upstream updates. You can use this command to see what has changed in the upstream repository and update your project accordingly.
...
When you add new components, the CLI will automatically use the correct theming classes based on your `components.json` configuration.
...
To use utility classes for theming set `tailwind.cssVariables` to `true` in your `components.json` file.
...
If you have `cssVariables` set to `true`, we will set the base colors as CSS variables in your `globals.css` file. If you have `cssVariables` set to `false`, we will use Tailwind CSS utility classes in your components.

---

### React Server Components

You can now opt out of using React Server Components. We will automatically append or remove the `use client` directive when adding components.
...
We are introducing a new concept called _Style_. You can think of style as a base. A style comes with its own set of components, animations, icons and more.

We are shipping with two styles: `default` and `new-york` (with more coming soon).
...
The `new-york` style is a new style. It ships with smaller buttons, cards with shadows and a new set of icons from [Phosphor Icons](https://phosphoricons.com/).

When you run the `init` command, you will be asked which style you would like to use. You can also change the style in your `components.json` file.
...
Start with a style then theme using CSS variables or Tailwind CSS utility classes to completely change the look of your components.
