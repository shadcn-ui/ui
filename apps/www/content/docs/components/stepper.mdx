...
The children accept a function that has as parameters the `methods` prop. The `methods` prop is an object with the `useStepper` hook from your stepper instance.
