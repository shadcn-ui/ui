...
<Step>Import the `<Slottable>` component in your `dropdown-menu.tsx` file</Step>

```diff showLineNumbers{3} title="dropdown-menu.tsx"
...
<Step>
  Wrap the `children` prop with `<Slottable>` in `<DropdownMenuCheckboxItem>`, `<DropdownMenuRadioItem>`, and `<DropdownMenuSubTrigger>` components:
</Step>

```diff showLineNumbers{105} title="dropdown-menu.tsx"
      </span>
-     {children}
...
```diff showLineNumbers{140} title="dropdown-menu.tsx"
      </span>
-     {children}
+     <Slottable>{children}</Slottable>
...
```diff showLineNumbers{217} title="dropdown-menu.tsx"
      {...props}
    >
-     {children}
+     <Slottable>{children}</Slottable>
