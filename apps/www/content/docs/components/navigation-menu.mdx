...
<Step>Import the `<Slottable>` component in your `navigation-menu.tsx` file</Step>

```diff showLineNumbers{3} title="navigation-menu.tsx"
...
<Step>
  Wrap the `children` prop with `<Slottable>` in `<NavigationMenu>`, and `<NavigationMenuTrigger>` components:
</Step>

```diff showLineNumbers{24} title="navigation-menu.tsx"
      {...props}
    >
-     {children}
...
```diff showLineNumbers{74} title="navigation-menu.tsx"
      {...props}
    >
-     {children}{" "}
+     <Slottable>{children}</Slottable>
