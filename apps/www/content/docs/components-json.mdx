...
The naming convention used for your component file names.

```json title="components.json"
{
  "case": "kebab" | "pascal" | "camel" | "snake"
}
```

<Callout className="mt-6">
  For example, if you set `case` to `pascal`, and add a component called `dropdown-menu`:
  ```bash
  npx shadcn-ui@latest add dropdown-menu
  ```
  the component will be added as `DropdownMenu.(tsx|jsx)` file.
</Callout>
