...
const Steps = ({ children, className, iconStyle, backgroundColor, borderColor, textColor, spanSize, divSize, steps, simple, anchorSize, bulletOfText, bulletStepText, panels, bullets, circle, bulletText, progressBar, icon, circleText, ...props }: StepsProps) => {
    ...
    const bordersColors = (borderColor?: colors) => {
                    ...
                    {steps.map((step: any, index: number) => (
                        <li key={index} className={cn("md:flex-1", className)}>
                            {step.status === 'complete' ? (
                                ...
                                <a
                                    href={step.href}
                                    className={cn("flex flex-col border-l-4 py-2 pl-4 md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4", bordersColors(borderColor), className)}
                                    aria-current="step"
                                >
                                    <span className={cn("text-sm font-medium", fontColor(textColor), bordersColors(borderColor), className)}>{step.id}</span>
                    ...
                    <div className="flex items-center justify-center	">
                        ...
                        <ol role="list" className={cn("ml-8 flex items-center space-x-5", className)}>
                            {steps.map((step: any) => (
                                <li key={step.name}>
                                    {step.status === 'complete' ? (
                                        <a href={step.href} className={cn(" hover:bg-primary/90", bgColor(backgroundColor), stepsVariants({}), className)}>
                                        </a>
                                    ) : step.status === 'current' ? (
                            ...
                            <li key={step.name} className={classNames(stepIdx !== steps.length - 1 ? 'pr-8 sm:pr-20' : '', 'relative')}>
                                {step.status === 'complete' ? (
                                    <>
                                        ...
                                        <a
                                            href="#"
                                            className={cn("hover:bg-primary/90", stepsCircleVariants({}), bgColor(backgroundColor), className)}>
                                            {icon && <Icon name={icon} className={`h-5 w-5 text-primary-foreground ${iconStyle}`} />}
                                        </a>
                                    </>
                                ) : step.status === 'current' ? (
                                    <>
                                        ...
                                        <a
                                            href="#"
                                            className={cn("border-2 bg-white", bordersColors(borderColor), stepsCircleVariants({}), className)} aria-current="step">
                                            ...
                                            {step.status === 'complete' ? (
                                                <a href={step.href} className="group">
                                                    <span className={cn("flex items-start", className)}>
                                                        <span className={cn("relative flex h-5 w-5 flex-shrink-0 items-center justify-center",bgColor(backgroundColor), stepsSpanVariants({ spanSize }), className)}>
                                                            {icon && <Icon name={icon} className={cn(`h-3 w-3 text-primary-foreground ${iconStyle}`, className)} />}
                                                        </span>
                                                        <span className={cn("ml-3 text-primary/90", stepsSpanVariants({}), className)}>
                                                            {step.name}
                                                        </span>
                                                    </span>
                                                </a>
                                                ...
                                                <a href={step.href} className="group">
                                                    <div className={cn("flex items-start", className)}>
                                                        <div className={cn("relative flex flex-shrink-0 items-center justify-center", stepsSpanVariants({ spanSize }), className)} aria-hidden="true">
                                                            <div className={cn("bg-primary/30 group-hover:bg-primary/40", stepsDivVariants({ divSize }), className)} />
                                                        </div>
                                                        <p className={cn("ml-3 text-primary/60 group-hover:text-primary", stepsSpanVariants({}), className)}>{step.name}</p>
                                                    </div>
                                                </a>
                                            )}
                                        </li>
                                    ...
                                    <li key={step.name} className={classNames(stepIdx !== steps.length - 1 ? 'pb-10' : '', 'relative')}>
                                        {step.status === 'complete' ? (
                                            <>
                                                ...
                                                <a href={step.href} className={cn("group relative flex items-start", className)}>
                                                    <span className={cn("flex items-center", className)}>
                                                        <span className={cn("z-10", stepsCircleVariants({}), bgColor(backgroundColor), className)}>
                                                            {icon && <Icon name={icon} className={cn(`h-5 w-5 text-primary-foreground ${iconStyle}`, className)} />}
                                                        </span>
                                                    </span>
                                            ...
                                            <>
                                                ...
                                                <a href={step.href} className={cn("group relative flex items-start", className)} aria-current="step">
                                                    <span className={cn("flex items-center", className)} aria-hidden="true">
                                                        <span className={cn("z-10 border-2 bg-white", bordersColors(borderColor), stepsCircleVariants({}), className)}>
                                            ...
                                            <>
                                                ...
                                                <a href={step.href} className={cn("group relative flex items-start", className)}>
                                                    <span className={cn("flex items-center", className)} aria-hidden="true">
                                                        <span className={cn("z-10 border-2 border-primary/20 bg-white group-hover:border-primary/40", stepsCircleVariants({}), className)}>
                                            ...
                                            <li key={step.name} className={cn("relative md:flex md:flex-1", className)}>
                                                {step.status === 'complete' ? (
                                                    <a href={step.href} className={cn("group flex w-full items-center", className)}>
                                                        <span className={cn("flex items-center px-6 py-4 ", stepsSpanVariants({}), className)}>
                                                            <span className={cn(stepsPanelVariants({}), bgColor(backgroundColor), className)}>
                                                                {icon && <Icon name={icon} className={cn(`h-6 w-6 text-primary-foreground ${iconStyle}`, className)} />}
                                                            </span>
                                                    ...
                                                    <a href={step.href} className={cn("flex items-center px-6 py-4 ", stepsSpanVariants({}), className)} aria-current="step">
                                                        <span className={cn("border-2", bordersColors(borderColor), stepsPanelVariants({}), className)}>
                                                    ...
                                                    <a href={step.href} className={cn("group flex items-center", stepsSpanVariants({}), className)}>
                                                        <span className={cn("flex items-center px-6 py-4 ", className)}>
                                                            <span className={cn("border-2 border-primary/30 group-hover:border-primary/40", stepsPanelVariants({}), className)}>
