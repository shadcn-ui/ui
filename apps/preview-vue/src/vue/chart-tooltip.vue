...
<template>
  ...
  >
    ...
    <div>
      ...
      <div
        :class="cn('grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl transition-all ease-in-out hover:-translate-y-0.5 w-[8rem]')"
      >
        <div class="font-medium">Page Views</div>
        <div class="grid gap-1.5">
          <div
            v-for="(item, i) in [{ name: 'Desktop', value: 186, fill: 'var(--chart-1)' }, { name: 'Mobile', value: 80, fill: 'var(--chart-2)' }]"
            :key="i"
          ...
          >
            ...
            <div class="flex flex-1 justify-between items-center leading-none">
              ...
              <span class="font-mono font-medium text-foreground tabular-nums">{{ item.value.toLocaleString() }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="items-end">
      ...
      >
        <div class="grid gap-1.5">
          <div
            v-for="(item, i) in [{ name: 'Chrome', value: 1286, fill: 'var(--chart-3)' }, { name: 'Firefox', value: 1000, fill: 'var(--chart-4)' }]"
            :key="i"
            class="flex w-full items-stretch gap-2"
          >
            <div
              class="w-0 shrink-0 rounded-[2px] border-[1.5px] border-dashed bg-transparent my-0.5"
              :style="{ borderColor: item.fill }"
            />
            <div class="flex flex-1 justify-between items-end leading-none">
              <div class="grid gap-1.5">
                <span class="text-muted-foreground">{{ item.name }}</span>
              </div>
              <span class="font-mono font-medium text-foreground tabular-nums">{{ item.value.toLocaleString() }}</span>
    ...
    <div class="hidden! md:flex!">
      <div
        :class="cn('grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl transition-all ease-in-out hover:-translate-y-0.5 w-[9rem]')"
      >
        <div class="font-medium">Page Views</div>
        <div class="grid gap-1.5">
          <div class="flex w-full items-stretch gap-2">
            ...
            <div class="flex flex-1 justify-between items-end leading-none">
              ...
              <span class="font-mono font-medium text-foreground tabular-nums">12,486</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="grid gap-1.5">
          <div class="flex w-full items-center gap-2">
            ...
            <div class="flex flex-1 justify-between items-center leading-none">
              ...
              <span class="font-mono font-medium text-foreground tabular-nums">1,286</span>
            </div>
          </div>
        </div>
      </div>
