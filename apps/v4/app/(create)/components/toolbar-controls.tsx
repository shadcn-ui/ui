...
const PROJECT_TYPES: ProjectTypeOption[] = [
  ...
  },
  {
    ...
    description: "Add shadcn/ui to an existing project",
  },
]
...
const TEMPLATES: Template[] = [
  {
    ...
    supportedProjectTypes: ["new", "existing"],
  },
  {
    value: "next-monorepo",
    title: "Next.js Monorepo",
    logo: '<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Next.js</title><path d="M18.665 21.978C16.758 23.255 14.465 24 12 24 5.377 24 0 18.623 0 12S5.377 0 12 0s12 5.377 12 12c0 3.583-1.574 6.801-4.067 9.001L9.219 7.2H7.2v9.596h1.615V9.251l9.85 12.727Zm-3.332-8.533 1.6 2.061V7.2h-1.6v6.245Z" fill="currentColor"/></svg>',
    supportedProjectTypes: ["existing"],
  },
  {
    value: "start",
...
export function ToolbarControls() {
  ...
  const [projectType, setProjectType] = React.useState<ProjectType>("new")
  ...
  const commands = React.useMemo(() => {
    ...
    const cliCommand = projectType === "new" ? "create" : "init"
    return {
      pnpm: `pnpm dlx shadcn@latest ${cliCommand} --preset "${url}"${templateFlag}`,
      npm: `npx shadcn@latest ${cliCommand} --preset "${url}"${templateFlag}`,
      yarn: `yarn dlx shadcn@latest ${cliCommand} --preset "${url}"${templateFlag}`,
      bun: `bunx --bun shadcn@latest ${cliCommand} --preset "${url}"${templateFlag}`,
    }
  }, [
    params.base,
    params.style,
    ...
    toast("Command copied to clipboard.", {
      description:
        projectType === "new"
          ...
          : "Paste and run the command in your terminal to add shadcn/ui to your existing project.",
  ...
  }, [command, params.template, projectType, setOpen])
  ...
  // Reset to first available template when changing project type if current is not supported
  const handleProjectTypeChange = React.useCallback(
    (value: string) => {
      const newProjectType = value as ProjectType
      setProjectType(newProjectType)

      // Check if current template is supported in new project type
      const currentTemplate = TEMPLATES.find(
        (t) => t.value === params.template
      )
      if (
        currentTemplate &&
        !currentTemplate.supportedProjectTypes.includes(newProjectType)
      ) {
        // Reset to first available template for this project type
        const firstAvailable = TEMPLATES.find((t) =>
          t.supportedProjectTypes.includes(newProjectType)
        )
        if (firstAvailable) {
          setParams({ template: firstAvailable.value as typeof params.template })
        }
      }
    },
    [params.template, setParams]
  )
      ...
      <DialogContent className="dialog-ring min-w-0 overflow-hidden rounded-xl sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-balance">
            {projectType === "new"
              ? `Select a template and run this command to create a ${selectedTemplate?.title} + shadcn/ui project.`
              : `Run this command to add shadcn/ui to your existing ${selectedTemplate?.title} project.`}
          </DialogDescription>
        </DialogHeader>
        <FieldGroup>
          <Field>
            <FieldLabel htmlFor="project-type" className="sr-only">
              Project Type
            </FieldLabel>
            <RadioGroup
              id="project-type"
            ...
            >
              {PROJECT_TYPES.map((type) => (
                ...
                >
                  <Field className="flex min-w-0 flex-col items-center justify-center gap-1 p-3! text-center *:w-auto!">
                    <RadioGroupItem
                      value={type.value}
          ...
          <Field>
            ...
            <RadioGroup
              ...
              className={`grid gap-2 ${
                filteredTemplates.length === 3 ? "grid-cols-3" : "grid-cols-4"
              }`}
            >
