...
import type { WidgetType } from "@/registry/bases/base/lib/rgl-dashboard-types"
import { Badge } from "@/registry/bases/base/ui/badge"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/registry/bases/base/ui/card"

/** Pre-written demo data for tiles (charts, tables, KPI copy). */
const kpiDemo = {
  revenue: { label: "Total revenue", value: "$48,234", delta: "+12.5%", positive: true },
  activeUsers: { label: "Active users", value: "2,847", delta: "+3.2%", positive: true },
  metric: { label: "Conversion", value: "3.24%", delta: "-0.4%", positive: false },
} as const
    ...
    default:
      return (
        <Card className="h-full border-dashed">
          <CardHeader>
            <CardTitle className="text-muted-foreground">Unknown widget</CardTitle>
          </CardHeader>
        </Card>
      )
  }
    ...
    <Card className="h-full border-border/80 bg-muted/30">
      <CardHeader className="flex flex-row items-center gap-4 space-y-0 pb-2">
        ...
        <div className="min-w-0 space-y-1">
          <CardTitle className="text-lg leading-tight">Dashboard demo</CardTitle>
          <CardDescription>
            CRUD dashboards, add or remove tiles, toggle <strong className="text-foreground">Edit layout</strong> to drag and
            resize. Tile ids match <code className="rounded bg-muted px-1 font-mono text-xs">layout[].i</code>.
          </CardDescription>
        </div>
      </CardHeader>
    </Card>
...
function KpiTile() {
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent>
        <span className={k.positive ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}>
          {k.delta}
        </span>
      ...
      <CardContent className="h-0 flex-1 pl-0">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartSeries} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <YAxis tickLine={false} axisLine={false} fontSize={11} tick={{ fill: "var(--muted-foreground)" }} width={32} />
            <Tooltip
              contentStyle={{
            ...
            />
            <Area type="monotone" dataKey="value" stroke="var(--primary)" fill={`url(#${fillId})`} strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
...
function BarChartTile() {
  return (
    <Card className="flex h-full flex-col">
      ...
      <CardContent className="h-0 flex-1 pl-0">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={barSeries} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid stroke="var(--border)" vertical={false} />
            <XAxis dataKey="region" tickLine={false} axisLine={false} fontSize={11} tick={{ fill: "var(--muted-foreground)" }} />
            <YAxis tickLine={false} axisLine={false} fontSize={11} tick={{ fill: "var(--muted-foreground)" }} width={28} />
            <Tooltip
              contentStyle={{
                background: "var(--popover)",
                border: "1px solid var(--border)",
        ...
        <table className="w-full caption-bottom text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="h-10 px-2 text-left font-medium text-muted-foreground">Product</th>
              <th className="h-10 px-2 text-left font-medium text-muted-foreground">Status</th>
              <th className="h-10 px-2 text-right font-medium text-muted-foreground">Revenue</th>
            </tr>
          </thead>
          <tbody>
            {tableRows.map((row) => (
              <tr key={row.id} className="border-b border-border/60">
                ...
                <td className="p-2 align-middle text-right tabular-nums">{row.revenue}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <Card className="h-full border-amber-500/30 bg-amber-500/5">
      ...
      <CardContent>
        ...
        <p className="text-muted-foreground text-xs mt-2">Pinned static tile in default layout.</p>
      </CardContent>
    </Card>
  )
}
