...
const WIDGETS: Record<string, { type: string }> = {
  "tile-kpi": { type: "kpi" },
      ...
      >
        {layout.map((item) => (
          ...
          >
            <TileByWidget
              id={item.i}
              type={(WIDGETS[item.i]?.type ?? "kpi") as "kpi" | "chart"}
            />
          </div>
        ))}
      </DashboardGrid>
      ...
      <Card className="border-dashed bg-muted/30">
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">
            Drag or resize tiles above —{" "}
            <code className="rounded bg-muted px-1 font-mono">
              layout[].x/y/w/h
            </code>{" "}
            updates in real time. This is the same shape passed to{" "}
            <code className="rounded bg-muted px-1 font-mono">
              onLayoutChange
            </code>
            .
