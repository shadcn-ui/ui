...
async function buildRegistryIndex(styles: Style[]) {
  // Build metadata (lightweight - no component loaders)
  let metadata = `/* eslint-disable @typescript-eslint/ban-ts-comment */
/* eslint-disable @typescript-eslint/no-explicit-any */
// @ts-nocheck
// This file is autogenerated by scripts/build-registry.ts
// Do not edit this file directly.
// 
// METADATA ONLY - Use this for generateStaticParams and metadata queries
// For component loaders, import from __loaders__.tsx

export const Metadata: Record<string, Record<string, any>> = {`

  // Build loaders (heavy - React.lazy component loaders)
  let loaders = `/* eslint-disable @typescript-eslint/ban-ts-comment */
/* eslint-disable @typescript-eslint/no-explicit-any */
// @ts-nocheck
// This file is autogenerated by scripts/build-registry.ts
// Do not edit this file directly.
...
export const Loaders: Record<string, Record<string, any>> = {`
  ...
  "${style.name}": {`
    ...
    for (const item of registry.items) {
      ...
      // Metadata entry (no component loader)
      metadata += `
    "${item.name}": {
      name: "${item.name}",
      ...
      // Loader entry (component loader only)
      loaders += `
    "${item.name}": ${
      componentPath
        ? `React.lazy(async () => {
  ...
  console.log(
    `#️⃣  Built multi-style registry with ${styles.length} styles: ${styles.map((s) => s.name).join(", ")}`
  )

  // Write metadata file (lightweight)
  rimraf.sync(path.join(process.cwd(), "registry/__metadata__.tsx"))
  await fs.writeFile(path.join(process.cwd(), "registry/__metadata__.tsx"), metadata)
  console.log(`   ✅ __metadata__.tsx (lightweight, ${(Buffer.byteLength(metadata) / 1024).toFixed(0)}KB)`)

  // Write loaders file (heavy)
  rimraf.sync(path.join(process.cwd(), "registry/__loaders__.tsx"))
  await fs.writeFile(path.join(process.cwd(), "registry/__loaders__.tsx"), loaders)
  console.log(`   ✅ __loaders__.tsx (heavy, ${(Buffer.byteLength(loaders) / 1024).toFixed(0)}KB)`)
}
