...
Use registry target placeholders `@components/`, `@ui/`, `@lib/` and `@hooks/` to refer to the corresponding aliases configured in `components.json`. These placeholders are independent of the project's import prefix.
