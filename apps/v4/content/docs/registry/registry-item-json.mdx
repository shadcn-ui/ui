...
| `registry:style`     | Use for registry styles. eg. `radix-force-ui`.    |
...
- For `Force UI` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
