...
<Callout>
  **For beta testers:** The namespace system is currently in beta. Use `npx
  shadcn@beta` for testing.
</Callout>
...
The namespace system operates on a trust model similar to npm:
...
4. **Access logs**: Monitor who's accessing your registry
5. **Content validation**: Validate resources before serving them
6. **Version immutability**: Never change a resource once published
