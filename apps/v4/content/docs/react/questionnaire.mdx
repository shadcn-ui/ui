...
Set `autoSubmit` on an item to continue after a fixed choice is selected.
`true` continues as soon as the item is answered. Pass a function to wait
until the selected values meet a condition. The last item submits the form.

Freeform typing never auto-submits. Arrow keys still browse radios without
advancing. Click, Space, and answer shortcuts do continue when `autoSubmit`
is set.
...
```tsx
<Questionnaire.Item autoSubmit name="task" required>
  {/* radios continue as soon as one is selected */}
</Questionnaire.Item>
...
<Questionnaire.Item
  ...
  required
>
  {/* checkboxes continue after two values are selected */}
</Questionnaire.Item>
```

`value` is the selected choice values as a `string[]`, in DOM order.
...
to the next item unless `autoSubmit` is set.
...
When `autoSubmit` is set, a click, Space, or answer shortcut on a fixed
choice continues automatically. Arrow keys still browse radios without
advancing.
...
- `autoSubmit` is opt-in. It continues after a choice is selected, which is a
  change of context. Progress already announces the new step. Arrow-key
  browsing does not skip items.
...
`QuestionnaireAutoSubmit` and the optional collection types are exported from
...
| `autoSubmit`     | `boolean \| ((value: string[]) => boolean)`       | -        | Continues after a qualifying choice selection. `true` continues once the item is answered.          |
