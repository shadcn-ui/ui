---
title: Accordion
description: A vertically stacked set of interactive headings that each reveal a section of content.
base: aria
component: true
links:
  doc: https://react-aria.adobe.com/DisclosureGroup
  api: https://react-aria.adobe.com/DisclosureGroup#api
---

<ComponentPreview
  styleName="aria-nova"
  name="accordion-demo"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[300px]"
/>

## Installation

<CodeTabs>

<TabsList>
  <TabsTrigger value="cli">Command</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```bash
npx shadcn@latest add accordion
```

</TabsContent>

<TabsContent value="manual">

<Steps className="mb-0 pt-2">

<Step>Install the following dependencies:</Step>

```bash
npm install react-aria-components
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource
  name="accordion"
  title="components/ui/accordion.tsx"
  styleName="aria-nova"
/>

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx showLineNumbers
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from "@/components/ui/accordion"
```

```tsx showLineNumbers
<Accordion defaultExpandedKeys={["item-1"]}>
  <AccordionItem id="item-1">
    <AccordionTrigger>Is it accessible?</AccordionTrigger>
    <AccordionContent>
      Yes. It adheres to the WAI-ARIA design pattern.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```

## Composition

Use the following composition to build an `Accordion`:

```text
Accordion
├── AccordionItem
│   ├── AccordionTrigger
│   └── AccordionContent
└── AccordionItem
    ├── AccordionTrigger
    └── AccordionContent
```

## Basic

A basic accordion that shows one item at a time. The first item is open by default.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-basic"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[300px]"
/>

## Multiple

Use the `allowsMultipleExpanded` prop to allow multiple items to be open at the same time.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-multiple"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[450px]"
/>

## Disabled

Use the `isDisabled` prop on `AccordionItem` to disable individual items.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-disabled"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[300px]"
/>

## Borders

Add `border` to the `Accordion` and `border-b last:border-b-0` to the `AccordionItem` to add borders to the items.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-borders"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[300px]"
/>

## Card

Wrap the `Accordion` in a `Card` component.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-card"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[435px]"
/>

## With Icon

Add an icon to the `AccordionTrigger` by rendering it as a child alongside the label. Icons without an explicit size default to `size-4`.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-with-icon"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[300px]"
/>

## Chevron Position

Use `data-chevron-position="left"` on the `AccordionTrigger` to place the chevron on the left instead of the right. It composes with custom icons.

<ComponentPreview
  styleName="aria-nova"
  name="accordion-chevron-left"
  align="start"
  previewClassName="*:data-[slot=accordion]:max-w-sm h-[300px]"
/>

## RTL

To enable RTL support in shadcn/ui, see the [RTL configuration guide](/docs/rtl).

<ComponentPreview
  styleName="aria-nova"
  name="accordion-rtl"
  align="start"
  direction="rtl"
/>

## API Reference

See the [React Aria](https://react-aria.adobe.com/DisclosureGroup#api) documentation for more information.
