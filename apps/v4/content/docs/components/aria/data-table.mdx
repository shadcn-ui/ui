...
TanStack Table v9 is feature-based: you opt into the behavior you want — sorting, filtering, pagination, and so on — by declaring it with `tableFeatures()`. Anything you don't list is tree-shaken out of your bundle.
...
// Declare the features this table uses once, then share the result across the
// column defs, the table instance, and any reusable components so the generics
// stay in sync. Only the features you list ship in the bundle.
...
```tsx showLineNumbers title="app/payments/columns.tsx" {3,5,16-19,21-31}
"use client"
...
// to the features you enabled. Use `accessor` for data columns and `display`
// for columns without an accessor (like selection or row-action columns).
...
```tsx showLineNumbers title="app/payments/data-table.tsx" showLineNumbers {3,9,16,22-25,32-47}
"use client"
...
```tsx showLineNumbers title="app/payments/data-table.tsx" {8,14,21-23,30,33,39-47}
"use client"
...
```tsx showLineNumbers title="app/payments/data-table.tsx" {9,15-20,30-31,39,43,58-100}
"use client"
