...
Vaul props like `handleOnly` and `shouldScaleBackground` do not have one-to-one
replacements in the Base UI drawer API. Use Base UI props such as
`disablePointerDismissal`, `modal`, `snapPoints`, or controlled `open` state for
...
`repositionInputs` is no longer needed. `DrawerPortal` renders Base UI's
`Drawer.VirtualKeyboardProvider`, and a bottom drawer moves up and shrinks to
stay above the on-screen keyboard while one of its inputs has focus.
