...
<template>
  <Calendar v-model="date" v-model:placeholder="placeholder" locale="fa-IR" />
  <!-- or -->
  <Calendar v-model="date" :default-placeholder="placeholder" locale="fa-IR" />
</template>
```
...
const date = ref(
  toCalendar(new CalendarDate(2025, 1, 1), new PersianCalendar())
)
// or
...
// or
const date = ref(
  toCalendar(fromDate(new Date(), getLocalTimeZone()), new PersianCalendar())
)
