...
| `cookieName`   | `string`                  | Cookie key used for persisted state.         |
| `shortcut`     | `string` or `null`        | Shortcut key; set `null` to disable.         |
...
By default, the `SidebarProvider` listens for `cmd+b` on Mac and `ctrl+b` on Windows.

If you have multiple sidebars in the same page, you can configure a different shortcut per provider using the `shortcut` prop.

```tsx showLineNumbers
<SidebarProvider shortcut="b">
  <Sidebar />
</SidebarProvider>
```

To disable the keyboard shortcut for a specific provider, set `shortcut={null}`. This opts that provider out of global keyboard handling.

```tsx showLineNumbers
<SidebarProvider shortcut={null}>
  <Sidebar />
</SidebarProvider>
```
...
If you have multiple sidebars and want them to persist independently, pass a unique `cookieName` to each provider.

If you only want one sidebar to respond to `cmd/ctrl + b`, set `shortcut={null}` on the other provider. You can also choose a custom shortcut (e.g. `shortcut="k"`) if you want the second sidebar to be keyboard-toggleable.
...
export async function Layout({ children }: { children: React.ReactNode }) {
  ...
  const leftDefaultOpen = cookieStore.get("sidebar_left_state")?.value === "true"
  const rightDefaultOpen = cookieStore.get("sidebar_right_state")?.value === "true"

  return (
    <>
      <SidebarProvider defaultOpen={leftDefaultOpen} cookieName="sidebar_left_state" shortcut="b">
        {/* Left sidebar */}
        {children}
      </SidebarProvider>

      <SidebarProvider defaultOpen={rightDefaultOpen} cookieName="sidebar_right_state" shortcut={null}>
        {/* Right sidebar */}
      </SidebarProvider>
    </>
  )
