...
To disable the keyboard shortcut for a specific provider, set `shortcut={null}`.
...
export async function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <SidebarProvider defaultOpen={rightDefaultOpen} cookieName="sidebar_right_state" shortcut="k">
        {/* Right sidebar */}
      </SidebarProvider>
    </>
  )
