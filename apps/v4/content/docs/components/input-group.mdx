...
To use your own custom input with `InputGroup`, add the `data-slot="input-group-control"` attribute to the input. The behavior and focus state will be handled automatically.

No style is applied to the custom input. You can apply your own styles using the `className` prop.
...
Use the `InputGroupInput` component as a replacement for `<Input />` when building input groups. This component has the input group styles pre-applied and uses the unified `data-slot="input-group-control"` for focus state handling.
...
Use the `InputGroupTextarea` component as a replacement for `<Textarea />` when building input groups. This component has the textarea group styles pre-applied and uses the unified `data-slot="input-group-control"` for focus state handling.
