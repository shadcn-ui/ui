...
Use `showSwipeHandle` on `Drawer` to render a swipe handle, or place `DrawerSwipeHandle` manually inside `DrawerContent`.
...
Set `modal={false}` to allow interaction with the rest of the page while the drawer is open. Combine with `disablePointerDismissal` to prevent the drawer from closing on outside presses.
...
<ComponentPreview styleName="base-rhea" name="drawer-rtl" direction="rtl" />
