...
Add a spinner to a button to indicate a loading state. Place the `<Spinner />` before the label for a start position or after the label for an end position.

<ComponentPreview styleName="radix-nova" name="spinner-button" />

### Badge

Add a spinner to a badge to indicate a loading state. Place the `<Spinner />` before the label for a start position or after the label for an end position.
