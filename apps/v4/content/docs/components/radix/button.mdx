...
Combine the `Button` component with fixed positioning to create a floating action button (FAB) — ideal for a primary action that stays visible while scrolling. This reuses the existing `Button`, so no extra component is needed. Change the corner with the `top`/`bottom` and `left`/`right` utilities, and swap `size` for a wider value if you need a text label alongside the icon.

<ComponentPreview styleName="radix-nova" name="button-floating" />
