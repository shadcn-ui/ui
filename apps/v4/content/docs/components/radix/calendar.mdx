...
To use the Persian calendar, edit `components/ui/calendar.tsx` and replace `@daypicker/react` with `@daypicker/persian`.
...
When using RTL, import the locale from `@daypicker/react/locale` and pass both the `locale` and `dir` props to the Calendar component:
...
Add `Locale` to your imports from `@daypicker/react`:
