...
A composed chart combines multiple chart types — such as `Area`, `Bar`, and `Line` — into a single `ComposedChart`. This is useful when you need to visualize related datasets that have different scales or units on the same graph.
...
<ChartContainer config={chartConfig}>
  <ComposedChart accessibilityLayer data={data}>
    ...
    <ChartTooltip content={<ChartTooltipContent />} />
    <Area yAxisId="left" dataKey="visitors" />
    <Bar yAxisId="left" dataKey="signups" />
    <Line yAxisId="right" dataKey="retention" />
  </ComposedChart>
</ChartContainer>
```

Key points when using `ComposedChart`:

- Use multiple `<YAxis>` components with `yAxisId` when your series have different scales (e.g. raw counts vs percentages).
- Assign the matching `yAxisId` prop to each `Area`, `Bar`, or `Line`.
- Layer order matters — components rendered later appear on top.

[Browse the Composed Charts gallery](/charts/composed).
