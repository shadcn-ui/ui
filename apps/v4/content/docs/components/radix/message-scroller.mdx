...
The copied `message-scroller.tsx` component is a thin styled wrapper over the
`@shadcn/react` package. All of the behavior lives in the package: anchoring,
follow-output, prepend preservation, visibility, and the imperative scroll hot
path.

If you are building your own design system, or you want full control over the
markup and styling, use the package directly instead of copying the component.
...
The frame mirrors the scroll-state attributes below (the viewport carries them
