...
Use the `container` prop on `SheetContent` to portal the sheet inside a specific DOM element instead of `document.body`. This is useful when using a sheet inside a sidebar layout to prevent it from covering the sidebar.

```tsx
const scrollRef = useRef<HTMLDivElement>(null)

<SheetContent side="bottom" container={scrollRef.current}>
  {/* portals inside the scroll container */}
</SheetContent>
```
