...
Use the `isFooter` prop mark a row as a footer.

<ComponentPreview styleName="react-aria-nova" name="table-footer" />

### Actions

A table showing actions for each row using a `<DropdownMenu />` component.

<ComponentPreview styleName="react-aria-nova" name="table-actions" />
