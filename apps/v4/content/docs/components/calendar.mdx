...
## Timezone Issue (Date Selecting Previous Day)

If you notice that selecting a date highlights the **previous day** (for example, selecting the 20th highlights the 19th), this is usually caused by *timezone differences*.

To fix this, you can pass the user’s **local timezone** to the Calendar component.
...
Then add the timeZone prop to your Calendar:
...
<Calendar
  ...
  selected={dateValue}
  timeZone={timeZone} // [!code highlight]
  captionLayout="dropdown"
/>
```

Adding the timeZone prop ensures the Calendar correctly aligns dates to the user’s local time, preventing off-by-one-day selection issues that can occur due to UTC conversion
