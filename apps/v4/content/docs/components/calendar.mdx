...
The `Calendar` component supports smooth animations when navigating between months using the `animate` prop from React DayPicker.
...
You can customize the animation duration and timing function using CSS variables:

```css
.my-calendar {
  --rdp-animation_duration: 0.5s;
  --rdp-animation_timing: ease-in-out;
}
```

**Accessibility:** The animations automatically respect the user's `prefers-reduced-motion` setting.
