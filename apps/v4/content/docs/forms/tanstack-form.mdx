...
- For combobox components with object items, the Combobox treats the entire object as its value. Map between the form's string value and the object using `value`/`onValueChange`.
- Use `itemToStringValue` to tell the Combobox how to display the object as a string.
- To show errors, add the `aria-invalid` prop to the `<ComboboxInput />` component and the `data-invalid` prop to the `<Field />` component.
...
```tsx showLineNumbers title="form.tsx" {6-7,11-13,15-17,23}
<form.Field
  name="language"
  children={(field) => {
    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid
...
The key pattern for bridging object-based Combobox items with a string form field:

1. **Find the object** from the form's string value: `languages.find((lang) => lang.value === field.state.value)`.
2. **Pass the object** to `value` on the Combobox: `value={selectedLanguage ?? null}`.
3. **Extract the string** in `onValueChange`: `field.handleChange(item?.value ?? "")`.
4. **Map the display** with `itemToStringValue`: `(lang: Language) => lang.label`.
