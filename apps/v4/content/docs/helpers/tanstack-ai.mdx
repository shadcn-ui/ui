---
title: TanStack AI
description: Create TanStack AI messages and stream predefined conversations through useChat without a model, API route, or network request.
---

`@shadcn/helpers/tanstack-ai` lets you write a conversation in code and play it
through TanStack AI's `useChat`.

Use it to build chat components, previews, docs, and tests without calling a
model or setting up an API route. It creates native TanStack `UIMessage[]`
values and streams responses through a local connection adapter.
...
| `fallback` | `string`, `MessagePart[]`, or a callback | —       | Response to stream when no predefined assistant response remains.             |
...
| ---------- | ----------------------------------------------------------- | ------- | ----------------------------------------------------------------------------- |
| `delayMs`  | `number`                                                    | `50`    | Delay between text and reasoning deltas. Use `0` or `undefined` to remove it. |
| `fallback` | `string \| MessagePart[] \| ({ writer, messages }) => void` | —       | Response used when no predefined assistant response remains.                  |
