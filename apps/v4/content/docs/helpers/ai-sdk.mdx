---
title: AI SDK
description: Create AI SDK messages and stream predefined conversations through useChat without a model, API route, or network request.
---

`@shadcn/helpers/ai-sdk` lets you write a conversation in code and play it
through `useChat`.

Use it to build chat components, previews, docs, and tests without calling a
model or setting up an API route. It supports reasoning, tools, data, files,
sources, and every other part type supported by the AI SDK.
...
```ts
chat.assistant(({ writer }) => {
  writer.custom("approval")
})
```

The message receives a `custom` part with the given `kind`.
...
| `fallback` | `string`, `UIMessagePart[]`, or a callback | —       | Response to stream when no predefined assistant response remains.             |
...
| ---------- | ------------------------------------------------------------- | ------- | ----------------------------------------------------------------------------- |
| `delayMs`  | `number`                                                      | `50`    | Delay between text and reasoning deltas. Use `0` or `undefined` to remove it. |
| `fallback` | `string \| UIMessagePart[] \| ({ writer, messages }) => void` | —       | Response used when no predefined assistant response remains.                  |
...
needs events between its input and result.
