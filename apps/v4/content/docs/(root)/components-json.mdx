...
For authenticated registries, use the canonical URL that serves the registry
item directly. Auth headers may not be forwarded across redirects that change
origin, such as an apex domain redirecting to `www`.
