...
Import alias for your components. The CLI uses this value to figure out where to
put components when adding them to your project, and to rewrite the imports of
any generated component that isn't a `ui` primitive (e.g. blocks and other
composed components).
...
Since `aliases.ui` falls back to `${aliases.components}/ui` when it isn't set,
changing `aliases.components` also moves the default install location for `ui`
components unless you set `aliases.ui` explicitly.

<Callout className="mt-6">
  If you never see files appear under this alias, it's expected: most
  registry items install as `ui` components (governed by `aliases.ui`) or as
  `lib`/`hooks` files. `aliases.components` only comes into play for
  non-primitive components, such as blocks, that ship their own composed
  component files.
</Callout>

### aliases.utils

Import alias for the `cn` utility function generated by the CLI (in
`lib/utils.ts` by default). Unlike `aliases.lib`, which points at a
**directory** for your `lib` files in general, `aliases.utils` points at the
**module** that exports `cn`, so the CLI knows exactly which import to
rewrite whenever a component uses `cn`.

Defaults to `@/lib/utils`.
...
If you keep your `cn` helper somewhere other than `lib/utils.ts`, for example
`@/lib/shared/cn`, point `aliases.utils` at it directly:
