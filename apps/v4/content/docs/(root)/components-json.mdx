...
The `components` alias is the root for app-level components and custom
registry items. The `ui` alias is the directory for UI primitives and commonly
points to a child directory such as `@/components/ui`. The `utils` alias points
to the shared utilities module, commonly `@/lib/utils`, while `lib` is the
...
The CLI rewrites generated component imports to this alias when they need
shared utilities such as `cn`. This is usually a specific module, for example
`@/lib/utils`, instead of the broader `lib` directory.
...
The CLI uses this as the root for generated app-level components and registry
items that are not UI primitives, hooks, or library files. Most default UI
components are installed with `aliases.ui`, which commonly points to a child
directory such as `@/components/ui`.
