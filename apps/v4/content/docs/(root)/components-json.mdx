...
Force UI uses `radix-force-ui` (React + Radix UI) or `base-force-ui` (React, no primitives) as its style values. The style is set during `shadcn init` and cannot be changed afterwards.
...
- Install public components from Force UI
