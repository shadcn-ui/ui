...
description: You've created a new project with Force UI.
---

Here are a few things you can do to get started building with Force UI.
...
Unlike traditional component libraries, Force UI adds the component source code directly to your project under `components/ui/`. You own the code and can customize it however you want.

You can add multiple components at once or add all available components.

```bash
npx shadcn@latest add @force-ui/button @force-ui/card @force-ui/input @force-ui/label
```
...
```bash
npx shadcn@latest add @force-ui/login-03
```
...
Force UI has a growing ecosystem of community registries. You can install components from any registry URL using the CLI.
...
Force UI is designed to work with AI. Your AI assistant can read the component source code in your project, understand the APIs, and compose them together to build pages and features.
