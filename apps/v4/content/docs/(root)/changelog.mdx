...
### Default style button icons

We shipped the `icon-sm` and `icon-lg` sizes for the **New York** button back
in September, but the `default` style was still missing those variants. This
release adds both icon sizes to the default style so that icon-only buttons
look consistent regardless of the style you pick.
...
Or manually add the new variants under `size` in `buttonVariants`:
...
Fixes: [#8757](https://github.com/shadcn-ui/ui/issues/8757)
