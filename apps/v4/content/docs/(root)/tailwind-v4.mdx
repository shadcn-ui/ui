---
title: Tailwind v4
description: How to use Force UI with Tailwind v4 and React 19.
---
...
- The upstream `default` and `new-york` styles are not available in Force UI. Use `radix-force-ui` or `base-force-ui` instead.
...
One of the major advantages of using `Force UI` is that the code you end up with is exactly what you'd write yourself. There are no hidden abstractions.
