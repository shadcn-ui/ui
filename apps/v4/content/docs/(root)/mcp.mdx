...
<Tabs defaultValue="claude">
  ...
  <TabsContent value="kiro" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx shadcn@latest mcp init --client kiro
       ```

    **Restart Kiro IDE** and try the following prompts:
       - Show me all available components in the shadcn registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the shadcn registry

    **Note:** You can check the MCP Server view in the Kiro feature panel to verify the connection.
...
To configure MCP in Kiro IDE, add the shadcn server to your project's `.kiro/settings/mcp.json` configuration file:

```json title=".kiro/settings/mcp.json" showLineNumbers
{
  "mcpServers": {
    "shadcn": {
      "disabled": false,
...
Once loaded, you should see the shadcn server in the MCP Server view in the Kiro feature panel with a list of available tools.

See the [Kiro MCP documentation](https://kiro.dev/docs/cli/mcp/) for more details.
