...
1. **Registry Connection** - MCP connects to configured registries (Force UI, private registries, third-party sources)
...
- **Force UI Registry** - The default registry with all Force UI components
...
<Callout>
  **Note:** No configuration is needed to access the standard Force UI registry.
</Callout>
