...
**Status:** snapshot updated 2026-05-08 after JES-95 merged. **Lane 3 parity sweep is structurally complete: 19/21 components 🟢 (90%).** The remaining 2 are intentionally 🔵 (Field, Label — no standalone Figma source-of-truth nodes). Tracks Linear `JES-91`. See [`storybook-figma-parity-standard.md`](./storybook-figma-parity-standard.md) for the working standard each row in this inventory is being measured against.
...
**Summary counts (after JES-95 / Batch D — final state):**

- 🟢 Done: **19** (Accordion, AlertDialog, Alert, Badge, Button, Card, Checkbox, Dialog, DropdownMenu, Input, Popover, Progress, RadioGroup, Select, Separator, Skeleton, Switch, Tabs, Tooltip)
- 🟡 Has experimental: **0**
- 🟠 Production-only: **0**
- 🔵 Blocked: **2** (Field, Label — **intentional and permanent**: no standalone Figma source-of-truth nodes per `figma-to-code-automation-roadmap.md` §6)

**The parity sweep is structurally complete.** 19/21 components 🟢 (90%) is the maximum achievable; the remaining 2 are 🔵 by design and will not move to 🟢 unless Figma publishes standalone Field/Label component nodes (no plans for that today). No further parity batches are required.
